...
fs.readFile("./public/file/data.txt", 'utf8', function (err,data) {
    if (err) {
    ...
    if(data[0] != today){
        var file4 = fs.createWriteStream("./public/file/data.txt");
        file4.write(today);
        file4.end();
        Scraping();
    }
...
function ScrapingMeteo(){
	...
	https.get(url, res => {
		...
		res.on("end", () => {
			...
			var adesso = body["currently"]["icon"];
			var temperatura = body["currently"]["temperature"];
			temperatura = parseInt((temperatura-32)/1.8)
			console.log(adesso,temperatura);
			fs.appendFile('./public/file/data.txt', "\n" + adesso + "_" + temperatura, function (err) {
  				if (err) throw err;
  				console.log('Saved!');
			});
		});
